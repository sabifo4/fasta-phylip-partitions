...
print OUT "Taxon\tLen_aln\tNum_gaps\tNum_X\tMissing_taxon\tperc_missdat\tperc_missX\n";
...
foreach my $line (@aln1){
	...
	# and screws the rest of the parsing
		...
		if( $line =~ /^[0-9]/ && $line !~ /[A-Za-z]/ ){
			...
			print "Taxon: ".$tmp_sp." | Num gaps: ".length( $str_gaps )." | Num char \"X\": ".length( $str_other )." | Missing taxon?: ".$is_missing." | % Missing data: ".$tmp_missdat." | % Missing char \"X\": ".$tmp_missother."\n";
...
print OUT2 "\nMissing char \"X\" before (total taxa): ".sprintf( "%.2f", ($missing_taxa_other_before*100) )." (".$num_sp.")\n";
print OUT2 "\nMissing char \"X\" after (present taxa): ".sprintf( "%.2f", ($missing_taxa_other_after*100) )." (".$count_no_missing.")\n";
