...
##
## Usage:
## <path_to_script>/concatenate_genes.pl <alignment_file> <species_list> <separator> <out_name>
...
# Get output for log2 
print OUT5 "Gene"."\t"."Number of species"."\t"."Number of missing species"."\t"."Total genes"."\t"."Name of missing sp"."\n";
	...
	if( $line =~ /^\s*$/ ){
		...
		if( $count_sp != $num_sp ){
			...
			# foreach my $val ( @missing_sp ){
				# print $val."";
			# }
			print "\nFor locus ".$count_genes." we have ".$count_sp." species\n";
			print "There is/are ".scalar(@missing_sp)." locus/loci missing!\n";
			...
			print OUT5 $count_genes."\t".$count_sp."\t".scalar(@missing_sp)."\t".($count_sp + scalar(@missing_sp)).""."\n";
		}
...
foreach my $sp_in_hash (sort keys %species_hash){
	...
	print "\nLength for species ".$sp_in_hash." is: ".length( $species_hash2{$sp_in_hash} );		
}
