...
print OUT "Taxon\tLen_aln\tNum_gaps\tNum_N\tMissing_taxon\tperc_missdat\tperc_missN\n";
...
my $str_other_no_missingtaxa = "";
my $str_other_missingtaxa = "";
...
foreach my $line (@aln1){
	...
	# and screws the rest of the parsing
		...
		if( $line =~ /^[0-9]/ && $line !~ /[A-Za-z]/ ){
			...
			$tmp_sp = $tmp_line[0];
			$str_gaps = $tmp_line[1];
			$str_other = $tmp_line[1];
			$str_gaps =~ s/[A-Za-z]//g;
			$str_gaps =~ s/\r//;
			$str_gaps =~ s/\n//;
			$str_other =~ s/[AGTCagtc]//g;
			$str_other =~ s/[-]//g;
			$str_other =~ s/\r//;
			$str_other =~ s/\n//;
			chomp( $str_gaps );
			...
			if ( length( $str_gaps ) == $len_aln ){
				...
				$str_other_missingtaxa .= $str_other;
			}else{
				...
				$str_other_no_missingtaxa .= $str_other;				
			}
			# print "seq: ".$str_gaps."\n";
			# Calculate missing data per taxon
			$tmp_missdat = length( $str_gaps )*100/$len_aln;
			$tmp_missother = length( $str_other )*100/$len_aln;
			print "Taxon: ".$tmp_sp." | Num gaps: ".length( $str_gaps )." | Num char \"X\" \(e.g., N\): ".length( $str_other )." | Missing taxon?: ".$is_missing." | % Missing data: ".$tmp_missdat." | % Missing char \"X\" \(e.g., N\): ".$tmp_missother."\n";
			print OUT $tmp_sp."\t".$len_aln."\t".length( $str_gaps )."\t".length( $str_other )."\t".$is_missing."\t".$tmp_missdat."\t".$tmp_missother."\n";
...
print "\nEq: num_gaps / ( num_taxa * len_aln ) --> "."(".length( $str_other_missingtaxa )."+".length( $str_other_no_missingtaxa ).")/((".$count_missing."+".$count_no_missing.")*".$len_aln.")\n";
my $missing_taxa_other_before =  (length( $str_other_missingtaxa )+length( $str_other_no_missingtaxa ))/(($count_missing+$count_no_missing)*$len_aln);
print "Avg missing data before removing missing taxa = ".$missing_taxa_before." = ".($missing_taxa_before*100)."%\n";
print "Eq: num_gaps / ( num_taxa * len_aln ) --> ".length( $str_other_no_missingtaxa )."/(".$count_no_missing."*".$len_aln.")\n";
my $missing_taxa_other_after = length( $str_other_no_missingtaxa )/($count_no_missing*$len_aln);
print "Avg missing data after removing missing taxa = ".$missing_taxa_after." = ".($missing_taxa_after*100)."%\n\n";
...
print OUT2 "\nNum_missing_taxa\tNum_present_taxa\tNum_total_taxa\tAvg_missX_with_missing_taxa\tAvg_missX_without_missing_taxa\n";
print OUT2 $count_missing."\t".$count_no_missing."\t".$num_sp."\t".$missing_taxa_other_before."\t".$missing_taxa_other_after."\n";
print OUT2 "\nEq. before removing missing taxa: num_gaps / ( num_taxa * len_aln ) --> "."(".length( $str_other_missingtaxa )."+".length( $str_other_no_missingtaxa ).")/((".$count_missing."+".$count_no_missing.")*".$len_aln.")\n";
print OUT2 "Eq. after removing missing taxa:  num_gaps / ( num_taxa * len_aln ) --> ".length( $str_other_no_missingtaxa )."/(".$count_no_missing."*".$len_aln.")\n";
print OUT2 "\nMissing char \"X\" \(e.g., N\) before (total taxa): ".sprintf( "%.2f", ($missing_taxa_other_before*100) )." (".$num_sp.")\n";
print OUT2 "\nMissing char \"X\" \(e.g., N\) after (present taxa): ".sprintf( "%.2f", ($missing_taxa_other_after*100) )." (".$count_no_missing.")\n";
